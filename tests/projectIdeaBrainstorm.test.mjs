...
test("brainstorm ideas summarize privately and reuse one idea for implementation", () => {
  ...
  assert.doesNotMatch(modal, /resetWorkspaceState\(\); close\(\)/);
  assert.doesNotMatch(modal, /useEffect\(\(\) => \{\n\s*if \(!open\) resetWorkspaceState\(\)/);
  assert.match(modal, /AI refinement and brainstorm summary runtimes intentionally survive closing the modal/);
  assert.match(modal, /targetKey: refinementTargetKey/);
  assert.match(modal, /sourceIdeaId/);
  assert.match(plansHook, /summaryTargetKey/);
  assert.match(plansHook, /summarySourceIdeaId/);
  assert.match(plansHook, /backgroundPlansCompleted/);
  assert.match(plansHook, /backgroundPlansFailed/);
  assert.match(plansHook, /controller is mounted at App scope/);
  assert.match(plansHook, /targetKey: input\.targetKey/);
  assert.match(plansHook, /sourceIdeaId: input\.sourceIdeaId/);
  assert.match(plansHook, /controller is mounted at App scope/);
  const refinementHook = readFileSync("src/renderer/src/hooks/useProjectIdeaRefinement.ts", "utf8");
  assert.match(refinementHook, /backgroundRefinementCompleted/);
  assert.match(refinementHook, /PROJECT_IDEA_REFINEMENT_RUNTIME_TERMINATED/);
  assert.match(refinementHook, /controller is mounted at App scope/);
