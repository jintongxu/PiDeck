				...
				<div className="flex min-h-0 flex-1">
					...
					<section className="min-w-0 flex-1 overflow-y-auto p-6">
						{(selected || editing) ? <div className="flex h-full flex-col gap-4">
							...
							<Textarea className="min-h-48 flex-1 resize-none" value={draft.body} placeholder={t("projectIdeas.bodyPlaceholder")} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} />
							...
							<div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => { setEditing(false); if (selected) setDraft({ title: selected.title, body: selected.body, tags: selected.tags.join(", ") }); }}>{t("projectIdeas.cancel")}</Button><Button type="button" disabled={!draft.title.trim()} onClick={() => void (selected ? save() : createIdea())}>{t("projectIdeas.save")}</Button></div>
