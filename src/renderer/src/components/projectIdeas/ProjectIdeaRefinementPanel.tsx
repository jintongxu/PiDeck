	...
	return (
		<section className="rounded-lg border border-primary/30 bg-primary/5 p-3">
			<div className="flex items-center justify-between gap-2">
				<div className="flex items-center gap-2 text-sm font-medium"><Sparkles className="size-4 text-primary" />{t("projectIdeas.refinementTitle")}{props.confirmed && <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-[11px] font-medium text-emerald-700 dark:text-emerald-300">{t("projectIdeas.refinementConfirmed")}</span>}</div>
			...
			<p className="mt-1 text-xs text-muted-foreground">{t("projectIdeas.refinementHint")}</p>
			{props.canUseContext && (
			...
			{value ? (
				<div className="mt-3 grid gap-2">
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementSummary")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.summary} onChange={(event) => props.onChange({ ...value, summary: event.target.value })} /></label>
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementProblem")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.problem} onChange={(event) => props.onChange({ ...value, problem: event.target.value })} /></label>
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementGoal")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.goal} onChange={(event) => props.onChange({ ...value, goal: event.target.value })} /></label>
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementOutcome")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.expectedOutcome} onChange={(event) => props.onChange({ ...value, expectedOutcome: event.target.value })} /></label>
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementScope")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.scope.join("\n")} onChange={(event) => props.onChange(updateList(value, "scope", event.target.value))} /></label>
					<label className="grid gap-1 text-xs font-medium">{t("projectIdeas.refinementAcceptance")}<Textarea disabled={props.disabled} className="min-h-16 resize-y text-sm font-normal" value={value.acceptanceCriteria.join("\n")} onChange={(event) => props.onChange(updateList(value, "acceptanceCriteria", event.target.value))} /></label>
